...
$sql="select * from `inquirys` {$where} order by `inquiry_status` desc,`id` desc ";
$query=sql_query($sql);
$j=0;
...
if($ie){
    $filename = iconv("utf-8","euc-kr","황금고추를 찾아라 사전접수 ".$title."목록.xls");
}else{
    $filename = "황금고추를 찾아라 사전접수 ".$title."목록.xls";
}
...
<table>
    <tr>
        <th colspan="6">황금고추를 찾아라 사전접수 목록</th>
    </tr>
    <tr>
        <th>번호</th>
        <th>이름</th>
