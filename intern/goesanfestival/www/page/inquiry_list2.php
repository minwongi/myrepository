...
$total=sql_fetch("select count(*) as cnt from `inquirys2`");
if(!$page)
    $page=1;
$total=$total['cnt'];
$rows=10;
...
$sql="select * from `inquirys2` order by `id` desc limit {$start},{$rows}";
$query=sql_query($sql);
$j=0;
...
<style>
    ...
    .inquiry_width h2{background-color:#fff;margin-top:20px;text-align: center;font-size:30px;padding:20px;-webkit-border-radius:6px 6px 0 0;-moz-border-radius:6px 6px 0 0;border-radius:6px 6px 0 0;border:1px solid #000;position: relative}
...
<div class="inquiry_width">
    <h2>속풀이 고추난타 사전접수 목록
        <input type="button" value="전체저장" onclick="location.href=g5_url+'/page/inquiry_list2_excel.php?type=all'">
        <input type="button" value="승인저장" onclick="location.href=g5_url+'/page/inquiry_list2_excel.php?type=confirm'">
    </h2>
    <div class="tab">
        <ul>
            <li onclick="location.href=g5_url+'/page/inquiry_list.php'">황금고추를 찾아라</li>
            <li class="active">속풀이 고추난타</li>
        </ul>
    </div>
    <div class="inquiry_list">
        ...
        if($total_page>1){
            ...
            <div class="num_list01">
                <ul>
                    <?php if($page!=1){?>
                        <li class="prev"><a href="<?php echo G5_URL."/page/inquiry_list2.php?page=".($page-1); ?>">&lt;</a></li>
                    <?php } ?>
                    <?php for($i=$start_page;$i<=$end_page;$i++){ ?>
                        <li class="<?php echo $page==$i?"active":""; ?>"><a href="<?php echo G5_URL."/page/inquiry_list2.php?page=".$i; ?>"><?php echo $i; ?></a></li>
                    <?php } ?>
                    <?php if($page<$total_page){?>
                        <li class="next"><a href="<?php echo G5_URL."/page/inquiry_list2.php?page=".($page+1); ?>">&gt;</a></li>
                    <?php } ?>
                </ul>
            </div>
            <?php
...
<script>
    function fninquiryUpdate(type,id){
        ...
        if(confirm(msg)){
            location.href=g5_url+'/page/inquiry_edit2.php?type='+type+"&id="+id;
        }
